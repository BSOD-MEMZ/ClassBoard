<template>
  <mdui-card class="block feed-block">
    ...
    <div v-if="feedData.updatedAt" class="tip">
      更新：{{ feedData.updatedAt }}
    </div>
    <div class="feed-list">
      <div
        v-for="(item, idx) in feedData.items.slice(0, 4)"
        :key="idx"
        class="feed-item"
      >
...
.feed-item {
  ...
  border: 1px solid
    color-mix(
      in srgb,
      rgb(var(--mdui-color-outline-variant)) 36%,
      transparent 64%
    );
  background: color-mix(
    in srgb,
    rgb(var(--mdui-color-surface-container-high)) 68%,
    transparent 32%
  );
  padding: 12px 14px;
  line-height: 1.7;
