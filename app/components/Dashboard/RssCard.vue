<template>
  <m3e-card v-if="rssData.items.length" class="block feed-block rss-block" variant="elevated">
    <div slot="header" class="block-title">
      {{ rssData.title || "RSS 资讯" }}
    </div>
    <div slot="content">
      <div v-if="rssData.updatedAt" class="tip">
        更新：{{ rssData.updatedAt }}
      </div>
      <div class="feed-list">
        <div
          v-for="(item, idx) in rssData.items.slice(0, 4)"
          :key="idx"
          class="feed-item"
        >
